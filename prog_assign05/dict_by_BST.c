...
#include <stdio.h>
#include <string.h>
#define NWORDS 200000
#define MAX 1100
#define BUFFER_LENGTH 30

typedef struct word {
	char name[BUFFER_LENGTH]; //단어 이름
	char class[BUFFER_LENGTH]; //품사
	char explain[MAX];         //단어 설명
...
int main() {
	...
	while (!feof(fp)) {
		...
		dict[n]->name[strlen(dict[n]->name) - 1] = '\0'; //공백 제거
		token = strtok(NULL, "()");
		strcpy(dict[n]->class, token);
		token = strtok(NULL, "()");
		if (token == NULL) { //품사가 명시되지 않은경우
			char str[MAX];
			strcpy(str, dict[n]->class);
			strcpy(dict[n]->explain, str + 1);
			strcpy(dict[n]->class, "\0");
		}
		else strcpy(dict[n]->explain, token + 1);
		dict[n]->explain[strlen(dict[n]->explain) - 1] = '\0'; //공백 제거
...
Tree *Delete_BTS(Tree **root, Tree *node) {
	...
	if (x != NULL) x->p = y->p;
	if (y->p == NULL) *root = x; //삭제할 노드가 root
...
Tree *BTS_Successor(Tree *node) {
	if (node->right != NULL) return find_min(node->right); //오른쪽 부트리의 최솟값
	Tree *p = node->p;
